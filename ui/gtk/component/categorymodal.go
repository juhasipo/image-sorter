...
func (s *CategoryModal) Show(parent gtk.IWindow, categories []*apitype.Category) {
	...
	for _, entry := range categories {
		iter := s.model.Append()
		s.model.SetValue(iter, 3, entry.Id())
		s.model.SetValue(iter, 0, entry.Name())
		s.model.SetValue(iter, 1, entry.SubPath())
		s.model.SetValue(iter, 2, entry.ShortcutAsString())
	}
...
func (s *CategoryModal) save() {
	s.sender.SendCommandToTopic(api.CategoriesSave, &api.SaveCategoriesCommand{
		Categories: s.getCategoriesFromList(),
	})
	s.modal.Hide()
}
func (s *CategoryModal) saveDefault() {
	s.sender.SendCommandToTopic(api.CategoriesSaveDefault, &api.SaveCategoriesCommand{
		Categories: s.getCategoriesFromList(),
	})
	s.modal.Hide()
}
